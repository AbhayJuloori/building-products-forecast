-- sql/databricks/02_branch_demand_summary.sql
...
WITH best_model_scores AS (
    ...
    FROM building_products.forecast_evaluation
...
test_horizon AS (
    SELECT
        DATE_TRUNC('week', MIN(week_start_date)) AS first_test_week,
        DATE_TRUNC('week', MIN(week_start_date)) + INTERVAL 13 WEEKS AS horizon_end_week,
        EXTRACT(YEAR FROM MIN(week_start_date)) AS first_test_year
    FROM building_products.forecasts_test
),
best_forecasts AS (
    SELECT
        f.branch_id,
        ...
        f.y_true,
        f.y_pred
    FROM building_products.forecasts_test AS f
    ...
    WHERE f.week_start_date >= h.first_test_week
      ...
      AND EXTRACT(YEAR FROM f.week_start_date) >= h.first_test_year
),
branch_rollup AS (
    SELECT
        bf.branch_id,
    ...
    LEFT JOIN building_products.sales_history AS sh
